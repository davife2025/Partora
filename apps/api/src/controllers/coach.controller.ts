...
// ─── GET /api/coach/ws-token ──────────────────────────────────────
// Issues a short-lived token the client exchanges for a WebSocket connection
export async function getWsToken(
  ...
  try {
    ...
    const expiresAt = Date.now() + 30_000; // 30-second window to connect
...
export function setupCoachWebSocket(server: Server) {
  ...
  server.on("upgrade", async (req, socket, head) => {
    ...
    if (!url.pathname.startsWith("/ws/coach")) {
      socket.destroy();
      return;
    }
  ...
  wss.on("connection", (ws: WebSocket, _req: any, userId: string, params: URLSearchParams) => {
    ...
    const context: CoachContext = {
      ...
      solfaText: params.get("solfa_text")   ?? undefined,
    };
