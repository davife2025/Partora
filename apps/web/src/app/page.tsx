...
const INPUT_MODES = [
  { href: "/analyse", emoji: <PenLine className="h-5 w-5" />, label: "Type Lyrics",   desc: "Paste lyrics + pick a key", variant: "soprano" as const, badge: "Quick"     },
  { href: "/upload",  emoji: <Upload  className="h-5 w-5" />, label: "Upload Audio",  desc: "Any MP3, WAV or AAC file",  variant: "alto"    as const, badge: "Auto key"  },
  { href: "/search",  emoji: <Search  className="h-5 w-5" />, label: "Search a Song", desc: "Find any song by name",     variant: "tenor"   as const, badge: "Catalogue" },
  { href: "/record",  emoji: <Mic     className="h-5 w-5" />, label: "Record Live",   desc: "Hum or sing a snippet",     variant: "bass"    as const, badge: "Live"      },
];
        ...
        <HomeClient>
          <div className="px-5 pb-6 space-y-6">
            ...
            <section aria-label="How would you like to analyse a song?">
              <h2 className="text-sm font-medium text-muted mb-3 uppercase tracking-wider">Analyse a song</h2>
              <div className="grid grid-cols-2 gap-3">
                {INPUT_MODES.map((mode) => (
                  <Link key={mode.href} href={mode.href}>
                    <Card variant={mode.variant} hoverable className="h-full">
            ...
            {recentSongs && recentSongs.length > 0 && (
              <section aria-label="Recent songs">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-sm font-medium text-muted uppercase tracking-wider">Recent</h2>
                  <Link href="/library" className="text-xs text-soprano hover:text-soprano/80 flex items-center gap-0.5 transition-colors">
